...
BEGIN
	...
	SET NOEXEC ON -- Skip execution of the remaining script
END
GO
...
BEGIN TRANSACTION
	...
	DECLARE @JobName NVARCHAR(128) = '$(Database) - Production ETL'
	
	-- Check if category exists
	IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name = N'Data Warehouse')
	BEGIN
		EXEC msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'Data Warehouse'
	END
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_job 
		@job_name = @JobName,
		@description = 'Loads data into Production tables', 
